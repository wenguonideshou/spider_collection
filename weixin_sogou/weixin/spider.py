...
from weixin.db import WeixinRequest, RedisQueue
...
class Spider():
    ...
    def error(self, weixin_request):
        ...
        print('请求失败次数', weixin_request.fail_time, 'URL地址', weixin_request.url)
    ...
    def schedule(self):
        ...
        while not self.queue.empty():
            ...
            print('正在请求', weixin_request.url)
            response = self.request(weixin_request)
            if response and response.status_code in VALID_STATUSES:
                results = list(callback(response))
                if results:
                    for result in results:
                        print('新结果', type(result))
